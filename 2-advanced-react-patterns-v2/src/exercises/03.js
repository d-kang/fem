...
// const On = ({ children }) => <ToggleContext.Consumer>{({ on }) => (on ? children : null)}</ToggleContext.Consumer>
// const Off = ({ children }) => <ToggleContext.Consumer>{({ on }) => (on ? null : children)}</ToggleContext.Consumer>
// const Button = (props) => <ToggleContext.Consumer>{({ on, toggle }) => <Switch on={on} onClick={toggle} {...props} />}</ToggleContext.Consumer>
...
class Toggle extends React.Component {
  ...
  static On = (props) => {
    ...
    return <ToggleContext.Consumer>
      {
        (consumerProps) => {
          console.log('On consumerProps: ', consumerProps);
          return (consumerProps.on ? props.children : null)
        }
      }
      </ToggleContext.Consumer>
  }
  static Off = (props) => {
    console.log('Off props: ', props);
    return <ToggleContext.Consumer>
      {
        (consumerProps) => {
          console.log('Off consumerProps: ', consumerProps);
          return (!consumerProps.on ? props.children : null)
        }
      }
    </ToggleContext.Consumer>
  ...
  static Button = (props) => {
    ...
    return <ToggleContext.Consumer>
      {
        (consumerProps) => {
          console.log('Button consumerProps: ', consumerProps);

          return <Switch on={consumerProps.on} onClick={consumerProps.toggle} {...props} />
        }
      }
    </ToggleContext.Consumer>
  }

  // static On = <ToggleContext.Consumer>{({ on }) => (on ? props.children : null)}</ToggleContext.Consumer>
  // static Off = <ToggleContext.Consumer>{({ on }) => (!on ? props.children : null)}</ToggleContext.Consumer>
  // static Button = (props) => <ToggleContext.Consumer>{(consumerProps) => <Switch on={consumerProps.on} onClick={consumerProps.toggle} {...props} />}</ToggleContext.Consumer>
  ...
  render() {
    ...
    return (
      <ToggleContext.Provider value={this.state} something={'hi'}>
        { this.props.children }
